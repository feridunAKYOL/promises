...
const hasVowel = (str) => !Array.from(str).every((char) => !/^[aeiou]$/.test(char.toLowerCase()));

const challenge5 = (str) =>
	new Promise((res, rej) => {
		setTimeout(() => {
			if (hasVowel(str)) {
		...
		.then((resolvedVal) => {
			resolvedVal.pass = resolvedVal.status === 10 && hasVowel(resolvedVal.value);
			return resolvedVal;
		})
		.catch((rejectedVal) => {
			rejectedVal.pass = rejectedVal.status === 20 && !hasVowel(rejectedVal.value);
			return rejectedVal;
		});
		...
		.then((results) =>
			results.reduce(
				(all, next) => {
					...
					all.status = all.status === 'error' ? 'error' : !next.pass ? 'failing' : all.status;
